...
#导入数据
data = pd.read_table("data/Lung Cancer/lung-cancer.data", header=None, delimiter=",") #(32,57)
...
np.savetxt('result/lung_cancer_fitness.csv', fitness.T, fmt='%.4f', delimiter=',')
np.savetxt('result/lung_cancer_error.csv', error.T, fmt='%.4f', delimiter=',')
np.savetxt('result/lung_cancer_num.csv', num.T, fmt='%.4f', delimiter=',')