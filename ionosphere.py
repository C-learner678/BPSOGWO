...
#导入数据
data = pd.read_table("data/Ionosphere/ionosphere.data", header=None, delimiter=",")

#标签数值化
le = preprocessing.LabelEncoder()
le.fit(data[34])
data[34] = le.transform(data[34])
...
np.savetxt('result/ionosphere_fitness.csv', fitness.T, fmt='%.4f', delimiter=',')
np.savetxt('result/ionosphere_error.csv', error.T, fmt='%.4f', delimiter=',')
np.savetxt('result/ionosphere_num.csv', num.T, fmt='%.4f', delimiter=',')