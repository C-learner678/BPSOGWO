...
#导入数据
data = pd.read_table("data/Connectionist Bench/sonar.all-data", header=None, delimiter=",")

#标签数值化
le = preprocessing.LabelEncoder()
le.fit(data[60])
data[60] = le.transform(data[60])
...
np.savetxt('result/connectionist_bench_fitness.csv', fitness.T, fmt='%.4f', delimiter=',')
np.savetxt('result/connectionist_bench_error.csv', error.T, fmt='%.4f', delimiter=',')
np.savetxt('result/connectionist_bench_num.csv', num.T, fmt='%.4f', delimiter=',')