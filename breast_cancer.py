...
#导入数据
data = pd.read_table("data/Breast Cancer/breast-cancer.data", header=None, delimiter=",") #(286,10)
...
for i in range(len(data)):
    if data["node-caps"][i] == "?":
        drop.append(i)
    elif data["breast-quad"][i] == "?":
        drop.append(i)
data = data.drop(drop) #277
...
np.savetxt('result/breast_cancer_fitness.csv', fitness.T, fmt='%.4f', delimiter=',')
np.savetxt('result/breast_cancer_error.csv', error.T, fmt='%.4f', delimiter=',')
np.savetxt('result/breast_cancer_num.csv', num.T, fmt='%.4f', delimiter=',')