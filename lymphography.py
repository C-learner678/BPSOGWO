...
#导入数据
data = pd.read_table("data/Lymphography/lymphography.data", header=None, delimiter=",")
...
np.savetxt('result/lymphography_fitness.csv', fitness.T, fmt='%.4f', delimiter=',')
np.savetxt('result/lymphography_error.csv', error.T, fmt='%.4f', delimiter=',')
np.savetxt('result/lymphography_num.csv', num.T, fmt='%.4f', delimiter=',')