...
#导入数据
data = pd.read_table("data/Tic Tac Toe/tic-tac-toe.data", header=None, delimiter=",")
...
np.savetxt('result/tic_tac_toe_fitness.csv', fitness.T, fmt='%.4f', delimiter=',')
np.savetxt('result/tic_tac_toe_error.csv', error.T, fmt='%.4f', delimiter=',')
np.savetxt('result/tic_tac_toe_num.csv', num.T, fmt='%.4f', delimiter=',')